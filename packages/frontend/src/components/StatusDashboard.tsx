...
export const StatusDashboard: React.FC = () => {
  ...
  const selectedRegion = useMemo(() => regions.find(r => r.region === selected) ?? regions[0], [regions, selected]);
      ...
      <div className="w-full">
        <header className="flex flex-col md:flex-row items-center justify-between mb-8 gap-6">
          <div className="text-center md:text-left">
            <h1 className="text-3xl font-extrabold tracking-tight mb-1">Realtime Endpoint Status</h1>
        ...
        <main className="grid gap-8 md:grid-cols-2">
            ...
            {!selectedRegion ? (
              ...
              <>
                <div className="grid grid-cols-2 gap-4 text-gray-800">
                  <Metric label="Avg CPU Load" value={(selectedRegion.summary.avgCpuLoad * 100).toFixed(1) + "%"} />
                  <Metric label="Avg Wait (ms)" value={selectedRegion.summary.avgWaitTime.toFixed(0)} />
                  <Metric label="Avg Connections" value={selectedRegion.summary.avgActiveConnections.toFixed(0)} />
                  <Metric
                    label="Latest Timestamp"
                    value={
                      selectedRegion.dataPoints.length
                        ? formatTime(selectedRegion.dataPoints[selectedRegion.dataPoints.length - 1].timestamp)
                        : "-"
                    }
                  />
                </div>
                ...
                <div className="mt-4">
                  ...
                  {selectedRegion.dataPoints.length ? (
                    <pre className="bg-gray-100 p-3 rounded-lg text-xs overflow-x-auto">
                      {JSON.stringify(selectedRegion.dataPoints[selectedRegion.dataPoints.length - 1], null, 2)}
                    </pre>
                  ) : (
          ...
          <section className="bg-white rounded-3xl p-6 shadow-lg md:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold">Trends — {selected?.toUpperCase() ?? "—"}</h2>
              <div className="text-sm text-gray-500">
                {selectedRegion ? `${selectedRegion.dataPoints.length} points` : ""}
              </div>
            </div>
